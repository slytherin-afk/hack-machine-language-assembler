...
fn remove_comments_blanks<'a>(lines: Lines<'a>) -> Vec<&'a str> {
    lines
        .into_iter()
...
fn get_symbols<'a>(lines: &Vec<&'a str>) -> Result<HashMap<&'a str, String>, String> {
...
fn translate_symbols_variables<'a>(
    lines: &Vec<&'a str>,
    symbols: &HashMap<&'a str, String>,
...
fn main() {
    let args = Arguments::build(env::args()).unwrap();
    let input_file = fs::read_to_string(&args.input_file_path).unwrap();
    let input_without_comments = remove_comments_blanks(input_file.lines());
    let symbols = get_symbols(&input_without_comments).unwrap();
    let variables = get_variables(&input_without_comments, &symbols).unwrap();
    let mid = translate_symbols_variables(&input_without_comments, &symbols, &variables);
}
